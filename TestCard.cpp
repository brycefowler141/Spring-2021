...
void testCard() {				// CreditCard test function
    ...
    wallet[3] = new CreditCard("5585 6584 4764 7986", "John Bowman", 6000); //test case for negative price
    ...
    for (int j = 1; j <= 16; j++) {			// make some charges
        ...
        wallet[3]->chargeIt(double(-100)); //negative price
    }

    cout << "Card payments:\n";
    for (int i = 0; i < 4; i++) {			// make more charges
        cout << *wallet[i];
